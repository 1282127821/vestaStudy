...
@Controller
public class 断点续传 {
    private static final Logger LOGGER = LoggerFactory.getLogger(断点续传.class);
    ...
    public void getVedio(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String path = "G:\\work\\soft\\development\\video.mp4";
...
//            logger.error(e.getMessage());
    ...
    @SuppressWarnings("all")
    @RequestMapping(value = "/getVedio2")
    public ResponseEntity<byte[]> getVedio2(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (file.exists()) {
                ...
                InputStream ins = new FileInputStream(file);
                bis = new BufferedInputStream(ins);
//                LimitInputStream limitInputStream = new LimitInputStream(ins, new BandwidthLimiter(1));
//                bis = new BufferedInputStream(limitInputStream);
                ...
                // 如果设设置了Content-Length，则客户端会自动进行多线程下载。如果不希望支持多线程，则不要设置这个参数。
                // Content-Length: [文件的总大小] - [客户端请求的下载的文件块的开始字节]
                headers.add("Content-Length", new Long(contentLength).toString());
